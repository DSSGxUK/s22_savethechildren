...
try:
    ...
    warnings.warn(
        "Necessary modules for autoencoder features not found - assumed not desired"
    )
...
def get_best_hyperparameters(
    ...
    class HyperModel(kt.HyperModel):
        def build(self, hp):
            ...
            model.add(
                Conv2D(8, (3, 3), activation="relu", padding="same", name="conv3")
            )
            model.add(MaxPooling2D((2, 2), padding="same", name="mp3"))
            model.add(
                Conv2D(8, (3, 3), activation="relu", padding="same", name="conv4")
            )
            model.add(MaxPooling2D((1, 1), padding="same", name="Encoder_Output"))
            model.add(
                Conv2D(8, (3, 3), activation="relu", padding="same", name="conv5")
            )
            model.add(UpSampling2D((1, 1), name="us1"))
            model.add(
                Conv2D(8, (3, 3), activation="relu", padding="same", name="conv6")
            )
            model.add(UpSampling2D((2, 2), name="us2"))
            model.add(
                Conv2D(16, (3, 3), activation="relu", padding="same", name="conv7")
            )
            model.add(UpSampling2D((2, 2), name="us3"))
            model.add(
                Conv2D(32, (3, 3), activation="relu", padding="same", name="conv8")
            )
    ...
    print(
        f"""best LR: {best_hps.get('learning_rate')}, best batch size: {best_hps.get('batch_size')}."""
    )
...
def get_trained_autoencoder(
    ...
    :param input_data: image input of size (num of samples, width, height, bands) - reshaped & imputed
                       input of convert_tiffs_to_image_dataset
    :type input_data: _type_
    :param batch_size: batch size for training, defaults to 128
    :type batch_size: int, optional
    :param learning_rate: learning_rate for Adam optimizer, defaults to 1e-3
    :type learning_rate: int, optional
    :param epochs: number of epochs for training, defaults to 100
    :type epochs: int, optional
    :param savedir: directory for saving model, defaults to None
    :type savedir: str, optional
    :param model_name: name of saved h5 model file, defaults to "autoencoder"
...
def get_encoded_features(
    ...
    :rtype: numpy array
    """
    trained_autoencoder = tensorflow.keras.models.load_model(
        Path(trained_autoencoder_dir) / (model_name + ".h5")
    )
    ...
    if gpu:
        encodings = intermediate_model.predict(
            pred_generator, use_multiprocessing=True, workers=6
        )
    else:
        encodings = intermediate_model.predict(
            pred_generator, use_multiprocessing=False
        )
...
def check_autoencoder_reconstruction(trained_autoencoder, input_data):
    ...
    for i in range(1, n + 1):
        ...
        plt.imshow(
            input_data[sample, :, :, band].reshape(
                input_data.shape[1], input_data.shape[1]
            )
        )
        ...
        plt.imshow(
            decoded_imgs[sample, :, :, band].reshape(
                input_data.shape[1], input_data.shape[1]
            )
        )
        ax.get_xaxis().set_visible(False)
